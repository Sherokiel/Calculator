...
class Application
{
    ...
    function run()
    {
        ...
        while ($times < 1) {
            ...
            if (in_array($command, SYSTEM_COMMANDS)) {
                $this->execute_system_command($command, $historyRepository);

                continue;
            }

            $argument1 = $this->read_operand('Enter fist number: ', $command);

            if ($command != '^' && $command != 'sr' ) {
                $argument2 = $this->read_operand('Enter second number: ', $command, true);
            } else {
                $argument2 = $this->read_operand('Enter exponent: ', $command);
            }
    ...
    function execute_system_command($command, $historyRepository)
    {
        switch($command) {
            ...
            case(HISTORY):
                $this->show_history($historyRepository);

                break;

            case(EXPORT_HISTORY):
                $this->history_to_txt($historyRepository);
        }
    }
    ...
    function show_history($historyRepository)
    {
        ...
        do {
            ...
            if ($showDateHistory == 'full') {
                $this->show_history_items($historyGroups);
                $isDataValid = false;
            ...
            if (!$isDataValid) {
                return $this->show_history_items([$showDateHistory => $historyGroups[$showDateHistory]]);
            }
        } while (!$isDataValid);
    ...
    function show_history_items($historyGroups)
    {
        foreach ($historyGroups as $date => $historyItems) {
            ...
            foreach ($historyItems as $historyItem) {
                $historyFunction = $this->write_history_line($historyItem);
